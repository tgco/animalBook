...
public class TutorialInventoryScreen extends ButtonScreenAdapter implements Screen {
	 ...
	 */
	public TutorialInventoryScreen(AnimalBookGame gameInstance, final TutorialScreen tutorialScreen) {
		super(gameInstance);
		...
		dnd.addTarget(new Target(eatZone) {
			...
			public void drop(Source source, Payload payload, float x, float y, int pointer) {
				if (payload.getObject() instanceof Consumable)
					tutorialScreen.getWorld().getPlayer().eat(((Consumable)payload.getObject()).getType().getHungerValue());
			}
		});
	}
		...
		for (int i = 0; i < Consumable.DropType.values().length; i++){
			...
			dnd.addSource(new Source(inventoryButton){
				...
				public Payload dragStart(InputEvent event, float x, float y,int pointer) {
					...
					if (tutorialScreen.getWorld().getPlayer().getInventory().removeItem(Consumable.DropType.values()[index])){
				...
				public void dragStop(InputEvent event, float x, float y,int pointer, DragAndDrop.Payload payload, DragAndDrop.Target target){
					if (target == null){
						tutorialScreen.getWorld().getPlayer().getInventory().addItem(new Consumable(Consumable.DropType.values()[index]));
					}
				}
			}
					);
	...
	private void updateHealthBar() {
		...
		batch.draw(red,Gdx.graphics.getWidth()/2 - .0435f*Gdx.graphics.getWidth(), Gdx.graphics.getHeight() - .069f*Gdx.graphics.getHeight(), 94*(tutorialScreen.getWorld().getPlayer().getHealth()/100), .028f*Gdx.graphics.getHeight());
	}
	...
	protected void updateInventoryScreenItems(int consumableIndex) {
		...
		fonts[consumableIndex].draw(batch,
				"x" + String.valueOf(tutorialScreen.getWorld().getPlayer().getInventory().getInventory().get(Consumable.DropType.values()[consumableIndex]).size),
